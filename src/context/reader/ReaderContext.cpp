...
void ReaderContext::showContextMenuTmpl()
{
  ...
  _context_menu->setPos(TFT_WIDTH - _context_menu->getWidth(), TFT_HEIGHT - _context_menu->getHeight() - DISPLAY_PADDING);
...
void ReaderContext::showBookDirsTmpl()
{
  ...
  _book_dirs_menu->setHeight(TFT_HEIGHT - DISPLAY_PADDING * 2);
  _book_dirs_menu->setItemHeight((_book_dirs_menu->getHeight()) / BOOK_DIR_ITEMS_NUM);
  _book_dirs_menu->setPos(DISPLAY_PADDING, DISPLAY_PADDING);

  _scrollbar = new ScrollBar(ID_SCROLL);
  layout->addWidget(_scrollbar);
  _scrollbar->setWidth(SCROLLBAR_WIDTH);
  _scrollbar->setHeight(TFT_HEIGHT - DISPLAY_PADDING * 2);
...
void ReaderContext::showBooksListTmpl()
{
  ...
  _books_list_menu->setHeight(TFT_HEIGHT - DISPLAY_PADDING * 2);
...
void ReaderContext::showReadTmpl()
{
  ...
  _page->setHeight(TFT_HEIGHT - DISPLAY_PADDING * 2 + 16);
  _page->setBackColor(COLOR_DARKGREY);
  _page->setTextColor(COLOR_WHITE);
  _page->setPos(0, DISPLAY_PADDING);
  ...
  _progress_lbl->setPos(DISPLAY_PADDING, TFT_HEIGHT - _progress_lbl->getHeight());
}
...
void ReaderContext::backPressed()
{
  if (_mode == MODE_BOOK_READ)
  {
    if (!_brightness_edit_en)
    {
      // _input.enableBtn(BtnID::BTN_UP); // TODO
